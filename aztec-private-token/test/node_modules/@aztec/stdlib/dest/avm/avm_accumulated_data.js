import { AVM_ACCUMULATED_DATA_LENGTH, MAX_L2_TO_L1_MSGS_PER_TX, MAX_NOTE_HASHES_PER_TX, MAX_NULLIFIERS_PER_TX, MAX_PUBLIC_LOGS_PER_TX, MAX_TOTAL_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX } from '@aztec/constants';
 ...
 */ export class AvmAccumulatedDataArrayLengths {
    ...
    static get schema() {
        ...
        }).transform(({ noteHashes, nullifiers, l2ToL1Msgs, publicLogs, publicDataWrites })=>new AvmAccumulatedDataArrayLengths(noteHashes, nullifiers, l2ToL1Msgs, publicLogs, publicDataWrites));
    }
    static fromBuffer(buffer) {
        const reader = BufferReader.asReader(buffer);
        return new AvmAccumulatedDataArrayLengths(reader.readNumber(), reader.readNumber(), reader.readNumber(), reader.readNumber(), reader.readNumber());
    }
    toBuffer() {
        return serializeToBuffer(this.noteHashes, this.nullifiers, this.l2ToL1Msgs, this.publicLogs, this.publicDataWrites);
    }
    static fromFields(fields) {
        const reader = FieldReader.asReader(fields);
        return new AvmAccumulatedDataArrayLengths(Number(reader.readField()), Number(reader.readField()), Number(reader.readField()), Number(reader.readField()), Number(reader.readField()));
    }
    toFields() {
        return [
