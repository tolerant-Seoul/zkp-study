 ...
 */ class ArgumentEncoder {
   ...
   */ #encodeBoundedVec(abiType, arg, name) {
        // First we encode the storage array
        {
            // Get the storage array type from the BoundedVec struct
            const storageField = abiType.fields.find((f)=>f.name === 'storage');
            ...
            if (arg.length > maxLength) {
                ...
                const preview = arg.slice(0, 3).map((x)=>{
                ...
                throw new Error(`Error encoding param '${name ?? 'unnamed'}': ` + `expected an array of maximum length ${maxLength} and got ${arg.length} instead: [ ${preview}${suffix} ]`);
            }
        ...
        {
            const lenField = abiType.fields.find((f)=>f.name === 'len');
